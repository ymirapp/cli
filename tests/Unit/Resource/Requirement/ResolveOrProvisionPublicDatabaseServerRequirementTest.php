...
class ResolveOrProvisionPublicDatabaseServerRequirementTest extends TestCase
{
    public function testFulfillProvisionsPublicDatabaseServerIfNoneFound(): void
    {
        ...
        $provisioner->shouldReceive('provision')->once()
                    ->with(\Mockery::type(\Ymir\Cli\Resource\Definition\DatabaseServerDefinition::class), $context, ['private' => false])
                    ->andReturn($server);

        $requirement = new ResolveOrProvisionPublicDatabaseServerRequirement('Which server?');
