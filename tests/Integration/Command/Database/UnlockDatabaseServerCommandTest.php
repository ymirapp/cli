...
use Ymir\Cli\Command\Database\UnlockDatabaseServerCommand;
...
class UnlockDatabaseServerCommandTest extends TestCase
{
    public function testUnlockDatabaseServer(): void
    {
        ...
        $this->bootApplication([new UnlockDatabaseServerCommand($this->apiClient, $this->createExecutionContextFactory([
            DatabaseServer::class => function () { return new DatabaseServerDefinition(); },
        ]))]);

        $tester = $this->executeCommand(UnlockDatabaseServerCommand::NAME, ['server' => '1']);

        $this->assertStringContainsString('Database server unlocked', $tester->getDisplay());
    }
}
