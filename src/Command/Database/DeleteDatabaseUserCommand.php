...
class DeleteDatabaseUserCommand extends AbstractDatabaseCommand
{
    ...
    protected function perform()
    {
        ...
        if ($users->isEmpty()) {
            ...
            $username = (string) $this->output->choice('Which database user would you like to delete', $users->pluck('username'));
        }
        ...
        if (!$this->output->confirm(sprintf('Are you sure you want to delete the "<comment>%s</comment>" database user?', $user['username']), false)) {
            return;
        }
        ...
        if (!$databaseServer['publicly_accessible']) {
            ...
            $this->output->writeln(sprintf('DROP USER IF EXISTS %s@\'%%\'', $user['username']));
        }
    }
}
